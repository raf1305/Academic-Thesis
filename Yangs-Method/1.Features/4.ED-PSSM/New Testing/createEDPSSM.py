...
#end2 = int(sys.argv[3])+1
t1=' 0'
t2=' 1'

file = open("PSSM-ED-Testingset_new.csv","a+")
...
for i in range(709, 749):
    filename = str(i)+".FASTA.pssm"
    os.system("python probability.py " + filename)
    filename = str(i)+".prob.txt"
    os.system("python EDPSSM.py " + filename + t1)
    os.system("del "+filename)
for i in range(749, 945):
    filename = str(i)+".FASTA.pssm"
    os.system("python probability.py " + filename)
    filename = str(i)+".prob.txt"
    os.system("python EDPSSM.py " + filename + t2)
    os.system("del "+filename)
